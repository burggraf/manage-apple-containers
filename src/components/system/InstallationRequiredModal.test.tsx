...
describe("InstallationRequiredModal", () => {
	it("should render when isOpen is true", () => {
		...
		expect(screen.getByText("Apple Container CLI Required")).toBeInTheDocument();
	});

	it("should not render when isOpen is false", () => {
		render(<InstallationRequiredModal isOpen={false} onClose={() => {}} />);

		expect(screen.queryByText("Apple Container CLI Required")).not.toBeInTheDocument();
	});

	it("should display explanation text about the dependency requirement", () => {
		render(<InstallationRequiredModal isOpen={true} onClose={() => {}} />);

		expect(screen.getByText(/Apple Container CLI is required/i)).toBeInTheDocument();
	...
	it("should display exit notice text", () => {
		...
		expect(screen.getByText(/Please restart after installing the CLI/i)).toBeInTheDocument();
	});
	...
	it("should attempt to open GitHub releases page when link button is clicked", async () => {
		...
		expect(open).toHaveBeenCalledWith("https://github.com/apple/container/releases/latest");
	});
